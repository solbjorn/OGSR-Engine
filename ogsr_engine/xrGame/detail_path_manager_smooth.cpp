...
IC bool coincide_directions(const Fvector2& start_circle_center, const Fvector2& start_tangent_point, float start_cross_product,
                            const Fvector2& dest_circle_center, const Fvector2& dest_tangent_point, float dest_cross_product)
{
...
bool CDetailPathManager::compute_tangent(const STrajectoryPoint& start, const SCirclePoint& start_circle, const STrajectoryPoint& dest,
...
bool CDetailPathManager::build_circle_trajectory(const STrajectoryPoint& position, xr_vector<DetailPathManager::STravelPathPoint>* path, u32* vertex_id,
                                                 const u32 velocity)
{
    const float min_dist = .1f;
    DetailPathManager::STravelPathPoint t;
    t.velocity = velocity;
...
bool CDetailPathManager::build_line_trajectory(const STrajectoryPoint& start, const STrajectoryPoint& dest, u32 vertex_id,
                                               xr_vector<DetailPathManager::STravelPathPoint>* path, u32 velocity)
{
...
bool CDetailPathManager::build_trajectory(const STrajectoryPoint& start, const STrajectoryPoint& dest, xr_vector<DetailPathManager::STravelPathPoint>* path,
...
bool CDetailPathManager::build_trajectory(STrajectoryPoint& start, STrajectoryPoint& dest, const SCirclePoint tangents[4][2], const u32 tangent_count,
                                          xr_vector<DetailPathManager::STravelPathPoint>* path, float& time, const u32 velocity1, const u32 velocity2,
                                          const u32 velocity3)
{
    time = flt_max;
    SDist dist[4];
...
bool CDetailPathManager::compute_trajectory(STrajectoryPoint& start, STrajectoryPoint& dest, xr_vector<DetailPathManager::STravelPathPoint>* path, float& time,
                                            const u32 velocity1, const u32 velocity2, const u32 velocity3, const EDirectionType direction_type)
{
...
bool CDetailPathManager::compute_path(STrajectoryPoint& _start, STrajectoryPoint& _dest, xr_vector<DetailPathManager::STravelPathPoint>* m_tpTravelLine,
                                      const xr_vector<STravelParamsIndex>& start_params, const xr_vector<STravelParamsIndex>& dest_params,
                                      const u32 straight_line_index, const u32 straight_line_index_negative)
{
    ...
    for (; I != E; ++I)
    {
        ...
        for (; i != e; ++i)
        {
            ...
            m_temp_path.clear();
            if (compute_trajectory(start, dest, m_tpTravelLine ? &m_temp_path : nullptr, time, (*I).index, real_straight_line_index, (*i).index,
                                   direction_type))
            {
...
bool CDetailPathManager::init_build(const xr_vector<u32>& level_path, u32 intermediate_index, STrajectoryPoint& start, STrajectoryPoint& dest,
...
bool CDetailPathManager::fill_key_points(const xr_vector<u32>& level_path, STrajectoryPoint& start, STrajectoryPoint& dest)
{
    ...
    for (int _i = 0, i = 0, n = (int)level_path.size() - 1, j = n, m = j; _i < n;)
    {
        ...
        {
            if ((j == n) &&
                !ai().level_graph().valid_vertex_id(ai().level_graph().check_position_in_direction(start_point.vertex_id, start_point.position, dest.position)))
            {
                m = j;
                j = (i + j) / 2;
            }
        ...
        if (i >= m - 1)
        {
            ...
            if (i == n)
            {
                if (ai().level_graph().valid_vertex_id(
                        ai().level_graph().check_position_in_direction(start_point.vertex_id, start_point.position, dest.position)))
                {
...
IC CDetailPathManager::STravelPoint CDetailPathManager::compute_better_key_point(const STravelPoint& point0, const STravelPoint& point1,
...
void CDetailPathManager::build_path_via_key_points(STrajectoryPoint& start, STrajectoryPoint& dest, xr_vector<STravelParamsIndex>& finish_params,
                                                   const u32 straight_line_index, const u32 straight_line_index_negative)
{
    ...
    for (B != E ? ++I : I; I != E; ++I)
    {
        ...
        bool succeed =
            compute_path(s, d, &m_path, m_start_params, last_point ? finish_params : m_start_params, straight_line_index, straight_line_index_negative);
        if (!succeed)
        {
            m_path.clear();
            return;
...
void CDetailPathManager::add_patrol_point()
{
    ...
    if ((m_path.size() > 1) && m_state_patrol_path && !fis_zero(extrapolate_length()))
    {
        ...
        t = m_path.back();
        t.position.add(v);
        ai().level_graph().create_straight_path<false>(m_path.back().vertex_id, ai().level_graph().v2d(m_path.back().position),
                                                       ai().level_graph().v2d(t.position), m_path, m_path.back(), false, false);
    }
}
