...
bool CALifeSwitchManager::synchronize_location(CSE_ALifeDynamicObject* I)
{
    ...
    if (!I->children.empty())
    {
        ...
        std::memcpy(test, I->children.data(), size * sizeof(ALife::_OBJECT_ID));
...
void CALifeSwitchManager::try_switch_online(CSE_ALifeDynamicObject* I)
{
    ...
    VERIFY2((ai().game_graph().vertex(I->m_tGraphID)->level_id() != ai().level_graph().level_id()) || !Level().Objects.net_Find(I->ID) ||
            ...
            xr::format("frame [{}] time [{}] object [{}] with id [{}] is offline, but is on the level", Device.dwFrame, Device.dwTimeGlobal, I->name_replace(),
                       I->ID));
